...
1. 共享集成分支使用 `dev`。
2. 新设备接入时先同步最新远程，再开始新任务。
3. 每个任务优先使用独立功能分支，避免多台设备同时改同一组文件。
4. `README.md`、`AGENTS.md`、`PROJECT_CONTEXT.md` 视为共享上下文，更新时尽量一起维护。
5. `.env`、依赖缓存、虚拟环境和构建产物保持本地化，不纳入版本控制。
6. 遇到冲突时，以远程最新状态为准，在功能分支上解决后再同步回 `dev`。
